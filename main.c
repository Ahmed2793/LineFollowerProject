...
void generalInit(void){
  ...
  PID->position =0;
  PID-> Kp = 0.0151; //0.0151 Proportional Gain
	PID-> Ki = 0.0031; //0.0031 Integral Gain
 ...
 // PID-> maxspeedr = 100;
	//PID-> maxspeedl = 100;
	PID-> MotorRightSpeedBase = 50; // Base speed 
	PID-> MotorLeftSpeedBase = 50; // Base speed
	PID-> Var_ARR = 10;
  PID-> lastError = 0;
//  PID-> i= 0;
//  PID-> k= 0;
...
void GPIOConfig (void)
{
	...
	////////////////////////////////////////////
	GPIOB->CRH |= 0x000000BB;// // PB0,1 ALTERNATE FUNCTION 50MHZ (TIMER2)
...
{
 RCC->APB2ENR |= RCC_APB2ENR_IOPBEN | RCC_APB2ENR_AFIOEN; 
 RCC->APB1ENR |= RCC_APB1ENR_TIM4EN; // enable timer3
 
 TIM4->CCER |= TIM_CCER_CC4E;	// capture/compare 4 enabled
 TIM4->CCER |= TIM_CCER_CC3E;	// capture/compare 3 enabled
 TIM4->CR1 |= TIM_CR1_ARPE; //Auto-reload preload enable
 TIM4->CCMR2 |= TIM_CCMR2_OC4M_1 | TIM_CCMR2_OC4M_2 | TIM_CCMR2_OC4PE; //lost 4
 TIM4->CCMR2 |= TIM_CCMR2_OC3M_1 | TIM_CCMR2_OC3M_2 | TIM_CCMR2_OC3PE; //lost 3
 
 //PWM freq = Fclk/PSC/ARR  72MHz/1000
 //PWM Duty = CCR4/ARR

 TIM4->PSC = 72-1; //72 MHz divided by:
 TIM4->ARR = 1000; //1000, to get 72e6/1e3
...
TIM4->CCR4= T_3_4; //duty cycle ch4 
TIM4->CCR3= T_3_3; //duty cycle ch3
TIM4->EGR |= TIM_EGR_UG; //
